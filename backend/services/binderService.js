...
async function updateQuantity(binderId, cardId, delta) {
  if (delta !== 1 && delta !== -1) {
  ...
  return withTransaction(async (client) => {
    ...
    const nextQty = Number(existing.rows[0].qty) + delta;
    if (nextQty < 0) {
      throw httpError(400, 'Quantity cannot go below zero');
    }
    ...
    await client.query(
      'UPDATE binder_cards SET qty = $3 WHERE binder_id = $1 AND card_id = $2',
      [binderId, cardId, nextQty]
    );
